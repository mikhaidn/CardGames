...
describe('CardBack', () => {
  test('renders with correct dimensions', () => {
    const { container } = render(<CardBack cardWidth={60} cardHeight={84} />);
  ...
  test('renders with red theme when specified', () => {
    const { container } = render(<CardBack cardWidth={60} cardHeight={84} theme="red" />);
  ...
  test('renders with custom image when provided', () => {
    ...
    const { container } = render(
      <CardBack cardWidth={60} cardHeight={84} theme="custom" customImage={customImage} />
    );
  ...
  test('is draggable when draggable prop is true', () => {
    const { container } = render(<CardBack cardWidth={60} cardHeight={84} draggable={true} />);
